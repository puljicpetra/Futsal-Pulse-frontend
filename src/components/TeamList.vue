        ...
        <div class="team-item-actions">
          ...
          <div v-if="isOwner" class="organizer-team-actions">
            <div v-if="reg.status === 'pending'">
              <button
                @click="updateRegistrationStatus(reg._id, 'approved')"
              ...
              <button
                @click="updateRegistrationStatus(reg._id, 'rejected')"
            ...
            <div v-else-if="reg.status === 'pending_withdrawal'">
              <button
                @click="updateRegistrationStatus(reg._id, 'withdrawal_approved')"
                class="btn-sm btn-approve"
...
const fetchRegistrations = async () => {
  ...
  try {
    const { data } = await apiClient.get(`/api/registrations?tournamentId=${props.tournamentId}`);
...
const updateRegistrationStatus = async (registrationId, newStatus) => {
  try {
    ...
    emit('feedback', { type: 'success', text: `Registration status updated to ${formatStatus(newStatus)}.` });
  } catch (err) {
...
.btn-withdraw:hover { background-color: #f5c6cb; }
