    ...
    <div class="team-detail-page">
        ...
        <div v-else-if="team" class="team-content">
            ...
            <header class="team-header">
                ...
                <div
                    v-if="removePlayerMessage.text"
                    :class="`feedback-message-global ${removePlayerMessage.type}`"
                >
            ...
            <main class="team-body-grid">
                <div class="grid-left-column">
                    <section class="players-section card">
                        <h2>
                            <i class="fas fa-users"></i> Player Roster ({{ team.players.length }}/8)
                        </h2>
                        <ul
                            class="players-list"
                            :class="{ 'two-columns': team.players.length > 4 }"
                        >
                            <li
                            ...
                            >
                                <div class="player-info">
                                    <img
                                        ...
                                        :src="getImageUrl(player.profile_image_url)"
                                        class="avatar-sm"
                                        alt="avatar"
                                    />
                                    ...
                                    <div class="player-name-details">
                                        ...
                                        <span v-if="player.full_name" class="player-username"
                ...
                <div class="grid-right-column" v-if="isCaptain">
                    <section v-if="team.players.length < 8" class="invite-section card">
                        ...
                        <div class="invite-form">
                            ...
                            <small class="form-text text-muted"
                                >Enter at least 2 characters to search.</small
                            >
                        </div>

                        <div
                            v-if="inviteMessage.text"
                            :class="`invite-feedback ${inviteMessage.type}`"
                        >
                            {{ inviteMessage.text }}
                        </div>
                    ...
                    <section v-else class="invite-section card">
                        ...
                        <div class="team-full-message">
                            ...
                            <p>
                                The maximum number of players (8) has been reached. You cannot
                                invite more players.
                            </p>
                        </div>
                    </section>
    ...
    if (window.confirm(`Are you sure you want to remove ${playerUsername} from the team?`)) {
        ...
        try {
            ...
            removePlayerMessage.value = {
                type: 'success',
                text: `${playerUsername} has been removed.`,
            }
        } catch (err) {
            removePlayerMessage.value = {
                type: 'error',
                text: err.response?.data?.message || 'Failed to remove player.',
            }
        } finally {
            setTimeout(() => {
                removePlayerMessage.value = { type: '', text: '' }
...
const deleteTeam = async () => {
    ...
    if (
        window.confirm(
            `Are you sure you want to permanently delete the team "${teamName}"? This action cannot be undone.`
        )
    ) {
        try {
            ...
            removePlayerMessage.value = {
                type: 'error',
                text: err.response?.data?.message || 'Failed to delete the team.',
            }
            setTimeout(() => {
                removePlayerMessage.value = { type: '', text: '' }
            }, 4000)
