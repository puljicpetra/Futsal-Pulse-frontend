    ...
    <div v-else-if="match" class="match-content">
      ...
      <header class="match-header">
        ...
        <div class="score-display">
          <span class="final-score">{{ finalScore(match).teamA }} - {{ finalScore(match).teamB }}</span>
          <div v-if="match.result_type === 'penalties' && match.penalty_shootout" class="penalty-result">
            Penalties: {{ match.penalty_shootout.teamA_goals }} - {{ match.penalty_shootout.teamB_goals }}
          </div>
        </div>
        <div class="team-banner">
      ...
      <div class="match-body">
        ...
        <div class="timeline-container">
          ...
          <template v-for="(event, index) in allEventsSorted" :key="event._id">
            <div v-if="shouldShowHeader(event, index)" class="period-header">
              {{ shouldShowHeader(event, index) }}
            </div>
            <div class="timeline-entry" :class="event.teamId === match.teamB._id ? 'right' : 'left'">
        ...
        <div v-if="penaltyEvents.length > 0" class="penalty-timeline">
           ...
           <div class="timeline-split">
              <div class="timeline-col">
                  <div v-for="event in teamA_penalties" :key="event._id" class="event-row-split">
                      <span>{{ getPlayerName(event.playerId) }}</span>
                      <i :class="getPenaltyIcon(event.outcome)"></i>
                  </div>
              </div>
              <div class="timeline-col right-aligned">
                  <div v-for="event in teamB_penalties" :key="event._id" class="event-row-split">
                      <i :class="getPenaltyIcon(event.outcome)"></i>
                      <span>{{ getPlayerName(event.playerId) }}</span>
                  </div>
              </div>
...
const fetchMatchDetails = async () => {
  ...
  error.value = '';
  try {
    const response = await apiClient.get(`/api/matches/${route.params.id}`);
...
const allEventsSorted = computed(() => match.value?.events?.sort((a,b) => a.minute - b.minute) || []);
const penaltyEvents = computed(() => match.value?.penalty_shootout?.events || []);
const teamA_penalties = computed(() => match.value?.penalty_shootout?.events?.filter(e => e.teamId === match.value.teamA._id) || []);
const teamB_penalties = computed(() => match.value?.penalty_shootout?.events?.filter(e => e.teamId === match.value.teamB._id) || []);
...
.fa-futbol, .fa-check-circle { color: #198754; }
.yellow-card { color: #ffc107; }
.red-card, .fa-times-circle { color: #dc3545; }
